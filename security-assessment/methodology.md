...
> **Purpose:** Complete, self-improving methodology for white-box security assessments. The reference implementation uses Splintarr (Python/FastAPI/Docker) as a worked example. Sections marked `[REFERENCE: Splintarr]` contain project-specific file paths and commands — replace for other targets using the "Adapting This Methodology" section.
>
...
2. **Sections marked `[REFERENCE: Splintarr]`** contain project-specific file paths, endpoints, and test commands — replace for other targets or use the "Adapting This Methodology" section
...
**[REFERENCE: Chronometry]** The chronometry-ai assessment found that Chrome URLs with OAuth codes, session tokens, and API keys were captured and stored in plaintext metadata files for 3 years. Timeline HTML files embedded full-resolution base64 screenshots creating 200-800 MB self-contained exfiltration packages per day. Total data accumulation: ~279 GB over the default 3-year retention.
...
**[REFERENCE: Chronometry]** LLM-generated summaries were rendered via `v-html` without sanitization. Prior annotation summaries were injected into subsequent LLM prompts via `{recent_context}`, creating a prompt injection propagation chain.
...
**[REFERENCE: Chronometry]** The dashboard loaded 5 external CDN scripts despite claiming "nothing leaves your machine." SECURITY.md listed "screenshot privacy concerns" as out of scope, but screenshots captured URLs with OAuth tokens — a credential exposure issue.
...
**[REFERENCE: Chronometry]** Chronometry -> Windows Recall -> Kevin Beaumont's TotalRecall research. This comparison revealed that chronometry shares every architectural flaw Recall was excoriated for and adds an unauthenticated web API.
...
This methodology was written for Splintarr v1.3.0 but the methodology is reusable. To adapt it for any codebase:
...
Paste the resulting inventory into Phase 1, replacing the Splintarr-specific checks.
...
The following improvements were applied after running this methodology against Splintarr v1.3.0.
...
### Lessons Learned: chronometry-ai Assessment (2026-03-06)

The following improvements were identified during the chronometry-ai assessment and led to the creation of Phase 0.5.

#### Additions (from chronometry-ai run)
...
#### Modifications (from chronometry-ai run)
...
#### Tool Workarounds (from chronometry-ai run)
